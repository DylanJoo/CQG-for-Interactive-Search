...
def generate(inputs, max_length, device, model, tokenizer, f):
    ...
    scores = model.get_crossattention_scores(batch['attention_mask'])

    # write outputs
    for k, o in enumerate(outputs):
        c_question_pred = tokenizer.decode(o, skip_special_tokens=True)
        f.write("QUESTION: "+source_questions[k]+'\n')
        f.write("PRED: "+c_question_pred+'\n')
        f.write("TRUTH: "+target[k]+'\n')
...
if __name__ == '__main__':
    ...
    dataset = load_dataset('json', data_files=args.jsonl_file)
