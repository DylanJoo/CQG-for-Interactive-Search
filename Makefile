INDEX_DIR=/home/jhju/indexes/wikipedia-topiocqa-lucene
CORPUS_DIR=~/datasets/wiki.dump.20201020
CORPUS=${CORPUS_DIR}/wiki_psgs_w200.jsonl
DATASET_DIR=/home/jhju/huggingface_hub/cqg4is

# According to contriever paper, 
# we concat the title in the beginning of contents.
preprocess_wiki_corpus:
	python3 src/tools/tsv_to_jsonl.py \
	  --path_tsv ${CORPUS_DIR}/wiki_psgs_w200.tsv \
	  --path_jsonl ${CORPUS} \
	  --add_title
...
retrieve_serp_clariq_bm25:
	python3 src/data_augmentation/retrieve_passages.py \
	    ...
	    --output ${DATASET_DIR}/clariq_provenances_bm25.jsonl \
	    --k1 0.9 --b 0.4 \
	    --index_dir ${INDEX_DIR} \
	    --k 100
...
construct_provenances_fidcqg:
	# Build training data from BM25 search
	...
	# Build training data from dense search
	# python3 src/data_augmentation/handler.py \
...
predict_provenances_convqa: 
	python3 src/inference_fidcqg.py \
	    ...
	    --output_file predictions/${DATASET}_cqpred.fidcqg.${RETRIEVER}.ovl.jsonl \
